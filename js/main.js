//페이지 로드 이벤트
window.addEventListener("load", ()=>{   //window 즉, 브라우저에 이벤트연결
    //이 이벤트문은 이미지소스까지 모드 로딩이 완료되어야 안쪽 코드를 실행한다.
        //플러그인 사이트 자체에서 아래 구조를 붙이기 한 뒤 필요한 값만 수정해서 사용하라고 제시되어있다.
    const grid = new Isotope("section", { //배치할 요소를 감싸고 있는 article의 부모 section요소명
       itemSelector: "article",//배치할 요소명 article
       columnWidth: "article", //넓이값을 구할 요소명 articl
